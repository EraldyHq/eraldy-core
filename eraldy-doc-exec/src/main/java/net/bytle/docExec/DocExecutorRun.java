...
public class DocExecutorRun {
  ...
  public List<DocExecutorResult> run(Path... paths) {
    ...
    Logs.setLevel(docExecutor.getLogLevel());
    ...
    for (int i = 0; i < docTests.size(); i++) {
      ...
      if (!files.isEmpty()) {
        ...
        for (int j = 0; j < files.size(); j++) {
          ...
          DocLog.LOGGER.info(docExecutor.getName(), "Replacing the file block (" + Log.onOneLine(docFileBlock.getPath()) + ") from the file (" + docUnit.getPath() + ")");
          targetDoc
            .append(eol)
            .append(fileContent)
            .append(eol);
      ...
      if (code != null && !code.trim().isEmpty()) {
        ...
        if (
          ...
          DocLog.LOGGER.info(docExecutor.getName(), "Running the code (" + Log.onOneLine(code) + ") from the file (" + docUnit.getPath() + ")");
          try {
          ...
          DocLog.LOGGER.info(docExecutor.getName(), "The run of the code (" + Log.onOneLine(code) + ") was skipped due to caching from the file (" + docUnit.getPath() + ")");
